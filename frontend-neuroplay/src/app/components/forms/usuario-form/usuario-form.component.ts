  ...
  ngOnInit() {
      ...
      if (id) {
          this.usuarioService.getById(parseInt(id)).subscribe(usuario => {
              ...
              this.form = this.fb.group({
                ...
                password: [usuario.password, [Validators.required, Validators.minLength(8), Validators.pattern(/^(?=.*[A-Z])(?=.*[!@#$&*.]).{8,}$/)]],
          ...
          this.form = this.fb.group({
            ...
            username: ['', [Validators.required, Validators.email], [emailValidator(this.usuarioService)]],
    ...
    save() {
      ...
      usuarioAction.pipe(
        ...
        Swal.fire({
          title: this.usuario ? '¡Actualización exitosa!' : '¡Creación exitosa!',
          text: `El usuario "${usuarioForm.nombre}" ha sido ${this.usuario ? 'actualizado' : 'creado'}.`,
          icon: 'success',
          confirmButtonText: 'Aceptar'
        }).then(() => {
